...
\begin{frame}{Tree Algorithm by Barnes and Hut, complexity $O(N\log N)$}
  ...
  \begin{itemize}
    ...
    \end{figure}
    \pause
    \item For each $T_{\ell, k}$ cell, compute weights $w_{\ell,k,m}$. \textbf{\textcolor{red}{COST?}}
  \end{itemize}
\end{frame}
...
\begin{frame}{Tree Algorithm by Barnes and Hut: Interaction List}
  ...
  \end{figure}
  \pause
  \item Then for a point $x$, the potential is approximately \textbf{\textcolor{red}{ACCURACY?}}
    \[
    u(x) \approx \sum_{\ell=1}^{O(\log N)}\sum_{m=0}^p w_{\ell,k(\ell),m} S_m\pr{x_{\ell, k(\ell)}^* - x}
    \]
  \pause
  \item \textbf{\textcolor{red}{TOTAL COST?}} Total cost is $O(N\log N)$
  \end{itemize}
\end{frame}
...
\begin{frame}{Results: Comparing Parallel Options}
    ...
    \begin{figure}
    \begin{center}
    \includegraphics[width=0.85\textwidth, angle=0]{{varying_m_n32768_p5_8}.png}
    \end{center}
    %\caption{S}
    \label{fig:comp_vers}
        \end{figure}
